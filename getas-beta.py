...
TRANSLATIONS = {
    "en": {
        ...
        "arg_v6": "Display IPv6 networks.",
        "arg_v6o": "Display only IPv6 networks.",
...
""",
    ...
    "ru": {
        ...
        "arg_v6": "Отображать IPv6 сети.",
        "arg_v6o": "Отображать только IPv6 сети.",
...
Скрипт анализирует автономные системы (AS) и маршруты с укрупнением IPv4 сетей.
...
def fetch_routes(as_number, ver=4):
    ...
    try:
        ...
        # Определяем ключевое слово в зависимости от версии
        route_key = "route:" if ver == 4 else "route6:"
        routes = []

        for line in result.stdout.splitlines():
            if line.lower().startswith(route_key):
                # Извлекаем только сам адрес (удаляем лишние пробелы)
                network = line.split(":", 1)[1].strip()
                
                try:
                    # Пробуем распарсить сеть
                    routes.append(ipaddress.ip_network(network, strict=False))
                except ValueError:
...
def arg_parse(lang="en"):
    ...
    parser.add_argument("-v6", action="store_true", help=translate("arg_v6", lang))
    parser.add_argument("-v6o", action="store_true", help=translate("arg_v6o", lang))
    return parser
...
def main():
    try:
        ...
        if input_value.isdigit():
            ...
            if args.no_merge == False or args.v6o:
                merged_routes = merge_networks(routes, args.tolerance)
                filtered_routes = filter_nested_networks(merged_routes)
            else:
            ...
            if args.r:
                    ...
                    if args.no_merge == False and not args.v6o:
                        print(translate("merged_networks", args.lang))
                    else:
                        print(translate("networks", args.lang))
